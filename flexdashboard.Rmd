...
plot1 = instacart |> group_by(aisle) |>
  ...
  geom_col(alpha = 0.9) +
  labs( # title = "Order Counts (for Aisles with >10k Orders)", # Title now comes from ### header
    x = "Order Count ",
    y = "Aisle") +
...
### Chart B {data-width=650}  # Changed from ## to ###
...
plot2 <- instacart |>
  ...
  geom_line(size = 1) +  
  labs(
    # title = "Mean Item Count by Hour of Day (Per Aisle)", # Title now comes from ### header 
...
### Chart C {data-width=350} # Changed from ## to ###
...
plot3 <- instacart |>
  ...
  labs(
    # title = "Order Density by Hour for the 3 Most Popular Items", # Title now comes from ### header
    x = "Product Name",
    y = "Hour of Day (0-23)",
    fill = "Total Orders"
  ) +
